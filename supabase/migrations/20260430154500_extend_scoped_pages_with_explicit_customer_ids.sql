-- Extend scoped page RPCs to accept explicit customer-id scope.
-- This lets frontend avoid chunked `.in(...)` fallbacks and always use DB pagination.
...
BEGIN
  ...
  RETURN QUERY EXECUTE format(
    $sql$
      ...
      explicit_scope AS (
        SELECT DISTINCT unnest(coalesce($13, ARRAY[]::uuid[])) AS customer_id
      ),
      scoped_customers AS (
