-- Returns by refund processed_at must include orders created outside the period
-- (Shopify attributes returns to refund date, not order create date).
...
-- Patch scope breakdown: separate in-period orders vs all scoped orders for returns.
...
GRANT EXECUTE ON FUNCTION public.shopify_analytics_returns_for_scope(timestamptz, timestamptz, uuid[], uuid[]) TO authenticated, service_role;
