-- Fix KPI cards showing 0 by handling admin as full-org scope
-- and keeping hierarchy-based rollups for manager/supervisor/salesperson.
...
AS $$
  WITH viewer_flags AS (
    SELECT public.has_role(_viewer_user_id, 'admin') AS is_admin
  ),
  scope_users AS (
    SELECT unnest(coalesce(public.get_user_scope_user_ids(_viewer_user_id), ARRAY[_viewer_user_id]::uuid[])) AS user_id
  ),
  scoped_customers AS (
    -- Admin: all customers. Others: customers assigned to scoped users.
  ...
  scoped_orders AS (
    SELECT DISTINCT o.id, coalesce(o.total, 0)::numeric AS total
    ...
    INNER JOIN scoped_customers sc
      ...
      OR (
        ...
        AND o.shopify_customer_id = (
          SELECT c.shopify_customer_id FROM public.shopify_customers c WHERE c.id = sc.customer_id
        )
      )
