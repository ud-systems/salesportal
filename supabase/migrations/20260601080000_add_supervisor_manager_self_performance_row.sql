-- Supervisor dashboard: fetch selected manager's own (non-downstream) performance row.
...
AS $$
  ...
  manager_customers AS (
    -- Direct assignment-based manager customers.
    ...
    UNION
    -- Name-matched customers for shops using textual owner fields instead of explicit assignments.
    SELECT DISTINCT c.id AS customer_id, c.shopify_customer_id
