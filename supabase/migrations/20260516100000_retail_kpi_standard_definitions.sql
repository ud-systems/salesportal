...
-- original_gross_sales = SUM(total) VAT-inclusive, voided -> 0
-- current_gross_sales = SUM(COALESCE(current_total, total)), voided -> 0
-- net_sales_ex_vat = SUM(COALESCE(current_total, total) - COALESCE(total_tax, 0)), voided -> 0
-- vat_collected = SUM(total_tax), voided -> 0
-- refunded_returned_value = SUM(GREATEST(total - COALESCE(current_total, total), 0)), voided -> 0
--
  ...
  admin_orders AS (
    SELECT
      ...
      CASE
        WHEN public.normalize_financial_status(o.financial_status) = 'voided' THEN 0::numeric
        ELSE greatest(
          coalesce(o.total, 0)::numeric - coalesce(o.current_total, o.total, 0)::numeric,
          0::numeric
        )
      END AS eff_refund,
