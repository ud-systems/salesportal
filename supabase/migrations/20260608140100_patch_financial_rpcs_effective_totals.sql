-- Patches financial KPI RPCs to use shopify_order_effective_* helpers (no false refunds on pending).
-- Depends on: 20260608140000_shopify_analytics_unification.sql
-- Use Shopify originalTotalPriceSet (stored as shopify_orders.original_total) for pre-adjustment gross
-- and refund/return deltas. totalPriceSet can converge to currentTotalPriceSet after refunds, which
-- made GREATEST(total - current_total, 0) ~ 0 and understated returns vs Shopify analytics.
-- Depends on: 20260516100000_retail_kpi_standard_definitions.sql

ALTER TABLE public.shopify_orders
  ADD COLUMN IF NOT EXISTS original_total NUMERIC(14,2);

COMMENT ON COLUMN public.shopify_orders.original_total IS 'Shopify Order.originalTotalPriceSet at creation; paired with current_total for refund KPIs.';
COMMENT ON COLUMN public.shopify_orders.total IS 'Shopify totalPriceSet (may match current after refunds). Prefer original_total for pre-adjustment gross when set.';

-- Best-effort backfill where we still have total > current_total (partial refunds) before webhook resync.
UPDATE public.shopify_orders o
SET original_total = o.total
WHERE o.original_total IS NULL
  AND o.total IS NOT NULL
  AND o.current_total IS NOT NULL
  AND o.total > o.current_total;

-- Standard retail KPI definitions (auditor-aligned):
-- original_gross_sales = SUM(COALESCE(original_total, total)) (original_total = Shopify originalTotalPriceSet at creation), voided -> 0
-- current_gross_sales = SUM(COALESCE(current_total, COALESCE(original_total, total))), voided -> 0
-- net_sales_ex_vat = SUM(COALESCE(current_total, COALESCE(original_total, total)) - COALESCE(total_tax, 0)), voided -> 0
-- vat_collected = SUM(total_tax), voided -> 0
-- refunded_returned_value = SUM(GREATEST(COALESCE(original_total,total) - COALESCE(current_total, COALESCE(original_total,total)), 0)), voided -> 0
--
-- Postgres cannot change RETURNS TABLE shape via CREATE OR REPLACE alone; drop dependents first.
...
DROP FUNCTION IF EXISTS public.get_salesperson_financial_breakdown_rows(uuid, text, timestamptz, timestamptz);
DROP FUNCTION IF EXISTS public.get_selected_salespeople_scope_metrics_timeseries(uuid, uuid[], timestamptz, timestamptz, text);
...
-- Align hierarchy timeseries revenue with current gross (fallback to original_total then total when current_total null).
