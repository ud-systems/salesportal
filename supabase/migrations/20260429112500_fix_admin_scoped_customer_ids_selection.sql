-- Fix scoped drilldown queries for admin-selected salespeople.
-- When admin passes explicit salesperson ids, honor that set instead of
-- deriving scope from viewer scope ids.
...
AS $$
  ...
  effective_salespeople AS (
    ...
    CROSS JOIN authz a
    WHERE a.is_admin
      AND NOT EXISTS (SELECT 1 FROM requested_salespeople)
