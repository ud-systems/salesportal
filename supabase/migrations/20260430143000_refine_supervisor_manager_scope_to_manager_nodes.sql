-- Ensure supervisor manager scope only includes manager nodes.
-- A manager node is either:
-- 1) a user with an explicit manager role, or
-- 2) a user who leads at least one manager->member hierarchy edge.
